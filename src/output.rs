...
pub mod csv_output {
    ...
    impl CSVController {
        ...
        pub fn new(dir_filepath: &str) -> Self {
            ...
            fs::create_dir_all(full_dirpath.as_path()).expect("Cannot create new directory in supplied location.");

            // .unwrap() here is fine as this code is related to initialization
            CSVController{
                perturbation_writer: csv::Writer::from_path(full_dirpath.join("pose_perturbations.csv")).unwrap(),
                object_parameters_writer: csv::Writer::from_path(full_dirpath.join("pose_object_parameters.csv")).unwrap(),
                solar_object_writer: csv::Writer::from_path(full_dirpath.join("pose_solar_objects.csv")).unwrap()
            }
        }
    ...
    impl SimulationOutput for CSVController{
        ...
        fn write_out_perturbation(&mut self, petrub_out: PerturbationOut) {
            self.perturbation_writer.serialize(petrub_out)
                .expect("Failed to write simulation perturbation data to the corresponding csv file.");
            // Unwrap here as this is a critical error
            self.perturbation_writer.flush().unwrap();
        }

        fn write_out_object_parameters(&mut self, object_params: SimulationObjectParameters) {
            self.object_parameters_writer.serialize(object_params)
                .expect("Failed to write simulation object parameters to the corresponding csv file.");
            // Unwrap here as this is a critical error
            self.object_parameters_writer.flush().unwrap();
        }

        fn write_out_solar_object(&mut self, solar_object: SolarObjectOut) {
            self.solar_object_writer.serialize(solar_object)
