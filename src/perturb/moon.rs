...
fn calculate_moon_gravity_perturbation(sim_obj: &mut SimobjT, env: &Environment) -> Array3d {
    let (r_0, r_tb, ob_0) = match sim_obj.state.soi {
        Solarobj::Sun => (
            ...
            Some(env.moon.model.state.coords),
        ...
        Solarobj::Moon => (env.moon.model.state.coords, None, env.moon.attr.obliquity),
    };
    ...
    let accel_ecliptic = if r_tb.is_some() {
        ...
        newton_gravitational_field(&sim_obj.state.coord_helio, &r_0, env.moon.attr.mass)
    };
